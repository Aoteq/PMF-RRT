...
class RRTBiasPlanner:
    ...
    def get_random_config(self, q_goal):
        return q_goal if np.random.rand() < self.goal_bias else np.random.uniform(JOINT_LIMITS[:, 0], JOINT_LIMITS[:, 1])
            ...
            if self.is_collision_free_path(nearest.config, q_new):
                ...
                if self.is_goal_reached(q_new, q_goal):
                    if self.is_collision_free_path(q_new, q_goal):
                        ...
                        print(f"RRT-Bias: Goal reached in {self.iterations+1} iterations.")
                        return path[::-1]
            if self.iterations % 500 == 0:
                print(f"Iteration: {self.iterations}, Tree size: {len(self.tree)}")
        print("RRT-Bias: Failed to find a path.")
        return []
...
def plot_joint_angles(path, speed, filename="joint_angles_rrt_bias.png"):
    if not path: return
...
def plot_trajectory(trajectory, filename="end_effector_trajectory.png"):
...
def main():
    ...
    planner = RRTBiasPlanner(robot_id, MAX_ITER, STEP_SIZE, GOAL_SAMPLE_RATE, GOAL_TOLERANCE)
    print("开始 RRT-Bias 路径规划...")
