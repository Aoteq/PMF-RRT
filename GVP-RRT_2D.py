...
def point_in_obstacle_no_count(state, circular_obstacles):
    """用于预处理栅格，不计入碰撞检测次数。"""
    if not (SPACE_X_RANGE[0] <= state[0] <= SPACE_X_RANGE[1] and
            SPACE_Y_RANGE[0] <= state[1] <= SPACE_Y_RANGE[1]):
        return True
...
def obstacle_clearance_no_count(state, circular_obstacles, x_range, y_range):
    """计算点到最近障碍物边界及工作空间边界的近似安全距离。"""
    x, y = state
    boundary_clearance = min(x - x_range[0], x_range[1] - x, y - y_range[0], y_range[1] - y)
    obs_clearance = float('inf')
    for obs_cx, obs_cy, obs_r in circular_obstacles:
        obs_clearance = min(obs_clearance, get_distance(state, [obs_cx, obs_cy]) - obs_r)
    return min(boundary_clearance, obs_clearance)


def distance_point_to_segment(point, segment_start, segment_end):
    """计算点到线段的距离，用于给靠近start-goal主方向的栅格适当加权。"""
    p = np.array(point)
    ...
    projection = a + t * ab
    return get_distance(p, projection)


def build_gvp_probability_map(start_pos, end_pos, obstacles, x_range, y_range, grid_size):
    """
    构造GVP-RRT的栅格变量概率图。
    返回：
    - cells: 每个栅格的[x_min, x_max, y_min, y_max]
    - probabilities: 与cells对应的采样概率
    """
    cells = []
    weights = []

    x_values = np.arange(x_range[0], x_range[1], grid_size)
    y_values = np.arange(y_range[0], y_range[1], grid_size)

    for x0 in x_values:
        for y0 in y_values:
            x1 = min(x0 + grid_size, x_range[1])
            y1 = min(y0 + grid_size, y_range[1])
            center = np.array([(x0 + x1) / 2.0, (y0 + y1) / 2.0])

            if point_in_obstacle_no_count(center, obstacles):
                continue

            goal_dist = get_distance(center, end_pos)
            clearance = obstacle_clearance_no_count(center, obstacles, x_range, y_range)
            line_dist = distance_point_to_segment(center, start_pos, end_pos)

            goal_term = math.exp(-goal_dist / max(GVP_GOAL_SCALE, 1e-6))

            clearance_term = np.clip(clearance / max(GVP_CLEARANCE_SCALE, 1e-6), 0.05, 1.0)

            line_term = math.exp(-line_dist / max(GVP_LINE_SCALE, 1e-6))

            weight = 0.20 + 0.25 * goal_term + 0.45 * clearance_term + 0.10 * line_term
            weight = max(weight, GVP_MIN_WEIGHT)
    ...
    if len(weights) == 0 or np.sum(weights) <= 0:
        raise ValueError("GVP probability map has no valid free grid cells.")
    probabilities = weights / np.sum(weights)
    return cells, probabilities


def sample_from_gvp_probability_map(cells, probabilities, x_range, y_range):
    """根据GVP概率图采样一个连续状态。"""
    if random.random() < GVP_SAMPLE_PROB:
        selected_index = np.random.choice(len(cells), p=probabilities)
        x0, x1, y0, y1 = cells[selected_index]
        return np.array([random.uniform(x0, x1), random.uniform(y0, y1)])
    else:
        return np.array([random.uniform(x_range[0], x_range[1]),
                         random.uniform(y_range[0], y_range[1])])


def gvp_rrt_planning(start_pos, end_pos, obstacles, space_x_range, space_y_range,
    ...
    gvp_cells, gvp_probabilities = build_gvp_probability_map(
        start_pos, end_pos, obstacles, space_x_range, space_y_range, GVP_GRID_SIZE
    )
    ...
    for i in range(max_iter):
        ...
        if rand_val < GOAL_BIAS:
            ...
            random_state = sample_from_gvp_probability_map(
                gvp_cells, gvp_probabilities, space_x_range, space_y_range
            )
        ...
        if path_is_free:
            ...
            if get_distance(new_node.state, end_node_state) <= goal_reach_threshold:
                if is_path_collision_free(new_node.state, end_node_state, obstacles, step_size):
                    ...
                    path_found = True
                    print(f"GVP-RRT: Goal reached at iteration {i + 1}!")
                else:
                    print(f"GVP-RRT: Near goal at iter {i + 1}, but final connection to exact goal blocked.")
        else:
        ...
        if visualize_progress and (i % 100 == 0 or path_found or (not path_is_free and i % 20 == 0)):
            plot_environment(node_list, start_pos, end_pos, obstacles, space_x_range, space_y_range,
                             title=f"GVP-RRT (Iter: {i + 1}) {'Col' if not path_is_free else ''}",
    ...
    if path_found:
        ...
        if visualize_progress:
            plot_environment(node_list, start_pos, end_pos, obstacles, space_x_range, space_y_range,
                             title=f"GVP-RRT - Failed (Iter: {max_iter})", is_final_plot=False)
...
if __name__ == "__main__":
    ...
    print("\nRunning GVP-RRT Algorithm with Circular Obstacles...")
    start_time = time.time()
    gvp_rrt_path, gvp_rrt_node_list, path_length, collision_checks, collision_count = gvp_rrt_planning(
    ...
    if gvp_rrt_path:
        ...
        print(f"GVP-RRT (Circular Obs) Path found with {len(gvp_rrt_path)} points.")
        final_title = "Path Planning Result using GVP-RRT"
        plot_environment(gvp_rrt_node_list, START_POS, END_POS, CIRCULAR_OBSTACLES, SPACE_X_RANGE, SPACE_Y_RANGE,
                         title=final_title,
                         final_path=gvp_rrt_path,
                         is_final_plot=True)
    else:
        print(f"找到路径耗费的时间: {elapsed_time:.2f} 秒")
        print(f"总碰撞检测次数: {collision_checks}")
        print(f"总碰撞次数: {collision_count}")
        print("GVP-RRT (Circular Obs) failed to find a path.")
        final_title = "GVP-RRT Failed to Find a Path"
        plot_environment(gvp_rrt_node_list, START_POS, END_POS, CIRCULAR_OBSTACLES, SPACE_X_RANGE, SPACE_Y_RANGE,
                         title=final_title,
                         is_final_plot=True)

    plt.savefig("gvp_rrt_path_result.pdf", format='pdf', bbox_inches='tight')
    plt.savefig("gvp_rrt_path_result.png", format='png', dpi=300, bbox_inches='tight')

    print("\nFinal plot saved as 'gvp_rrt_path_result.pdf' and 'gvp_rrt_path_result.png'")
