...
RECT_OBSTACLE_POS = [0.7, -0.1, 5.45]
RECT_OBSTACLE_DIM = [0.7, 0.1, 0.9]
PLAYBACK_SPEED = 0.1
GOAL_BIAS = 0.00

DD_INITIAL_DOMAIN_RADIUS = float(np.linalg.norm(JOINT_LIMITS[:, 1] - JOINT_LIMITS[:, 0]))
DD_MIN_DOMAIN_RADIUS = STEP_SIZE * 1.5
...
class RRTNode:
    def __init__(self, config: np.ndarray, parent=None, domain_radius=DD_INITIAL_DOMAIN_RADIUS):
        self.config = config
        self.parent = parent
...
class DynamicDomainRRTPlanner:
    ...
    def nearest_node_in_dynamic_domain(self, q_rand):
        nearest = self.nearest_node(q_rand)
        if np.linalg.norm(nearest.config - q_rand) <= nearest.domain_radius:
            return nearest
        return None
    ...
    def plan(self, q_init, q_goal):
        ...
        for self.iterations in range(self.max_iter):
            ...
            nearest = self.nearest_node_in_dynamic_domain(q_rand)
            ...
            if self.is_collision_free_path(nearest.config, q_new):
                nearest.domain_radius = min(
                    DD_INITIAL_DOMAIN_RADIUS,
                    nearest.domain_radius * DD_SUCCESS_RECOVERY_FACTOR
                )
                ...
                if self.is_goal_reached(q_new, q_goal):
                    if self.is_collision_free_path(q_new, q_goal):
                        ...
                        print(f"Dynamic-domain RRT: Goal reached in {self.iterations + 1} iterations.")
                        return path[::-1]
            else:
                nearest.domain_radius = max(
                    DD_MIN_DOMAIN_RADIUS,
                    nearest.domain_radius * DD_SHRINK_FACTOR
                )
            if self.iterations % 500 == 0:
                print(f"Iteration: {self.iterations}, Tree size: {len(self.tree)}")
        print("Dynamic-domain RRT: Failed to find a path.")
        return []
...
def plot_joint_angles(path, speed, filename="joint_angles_dynamic_domain_rrt.png"):
    if not path:
        return
    ...
    plt.title("Dynamic-domain RRT 关节角变化曲线")
    plt.xlabel("时间（秒）")
    plt.ylabel("角度（弧度）")
    plt.grid(True)
    plt.legend()
...
def plot_trajectory(trajectory, filename="end_effector_trajectory_dynamic_domain_rrt.png"):
...
def main():
    ...
    planner = DynamicDomainRRTPlanner(robot_id, MAX_ITER, STEP_SIZE, GOAL_SAMPLE_RATE, GOAL_TOLERANCE)
    ...
    print("开始 Dynamic-domain RRT 路径规划...")
