...
class RRTPlanner:
    def __init__(self, env, step_size=0.9, max_iterations=5000, goal_reach_threshold=1.5, goal_sample_rate=0.1):
        self.env = env
    ...
    def plan(self, visualize_progress=True):
        ...
        for i in range(self.max_iterations):
            ...
            direction = q_rand - nearest_node.state
            dist = np.linalg.norm(direction)
            if dist < 1e-6:
                continue
            unit_direction = direction / dist
            new_state = nearest_node.state + self.step_size * unit_direction
            if dist <= self.step_size:
                new_state = q_rand

            path_is_free = self.env.is_collision_free(nearest_node.state, new_state, self.step_size)

            current_parent_for_plot = nearest_node.state
            ...
            if path_is_free:
                ...
                if dist_to_goal <= self.goal_reach_threshold:
                    if self.env.is_collision_free(new_node.state, self.env.goal, self.step_size):
                        ...
                        path_found = True
                        final_iteration = i + 1
                        print(f"RRT (Iter: {i+1}): Goal region reached and final connection successful!")
                    else:
                        print(f"RRT (Iter: {i+1}): Near goal, but final connection to exact goal failed.")

            else:
                collision_start_for_plot = nearest_node.state
                collision_end_for_plot = new_state

            if visualize_progress and (i % 100 == 0 or path_found or (not path_is_free and i % 10 == 0)):
                title_str = f"RRT (Iter: {i+1})"
                if not path_is_free:
                    title_str += " Col"
        ...
        if path_found and final_goal_node:
            ...
            print(f"总碰撞检测次数: {collision_check_count}")
            print(f"总碰撞次数: {collision_count}")
            print(f"RRT: Path found with 0 points.")
            if visualize_progress:
    ...
    def plot_environment(self, iteration, final_path=None, current_new_node_state=None,
        ...
        ax.set_ylabel('Y')
        ax.set_zlabel('Z')
        plt.title(f'RRT (Iteration: {iteration})')
        ax.legend()
        plt.pause(0.001)
